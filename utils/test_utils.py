...
def normalize_augment(datain):
	...
	'''
	def transform(sample, gt, density_map):
		...
		return transf[0](sample), transf[0](gt), transf[0](density_map)

	img_train = datain[0]
	gt_train = datain[1]
	density_map = datain[-1]
	...
	#gt_train = gt_train / 255
	#augment
	img_train, gt_train, density_map = transform(img_train, gt_train, density_map)

	return img_train, gt_train, density_map
class MixUp_AUG:
    def __init__(self):
...
def open_sequence(seq_dir, gray_mode, expand_if_needed=False, max_num_fr=300):
...
def blocks2image(blocks, img_height, img_width, block_rects, model_height, model_width, pad, upscale):
	assert len(blocks) > 0
	h_reduction_ratio = max(img_height // model_height, 1)
	w_reduction_ratio = max(img_width // model_width, 1)
	C, _, _ = blocks[0].shape
	concate_x = torch.zeros([1, C, img_height * upscale, img_width * upscale]).type_as(blocks[0])
	for i in range(h_reduction_ratio):
		for j in range(w_reduction_ratio):
			index = i * w_reduction_ratio + j
			block, block_index = blocks[index], block_rects[index]
			y_min, y_max, x_min, x_max = block_index

			pad_y_min = 0 if y_min == 0 else pad
			pad_y_max = 0 if y_max == img_height else pad
			pad_x_min = 0 if x_min == 0 else pad
			pad_x_max = 0 if x_max == img_width else pad

			y_min = (y_min + pad_y_min) * upscale
			y_max = (y_max - pad_y_max) * upscale
			x_min = (x_min + pad_x_min) * upscale
			x_max = (x_max - pad_x_max) * upscale
			c, h, w = block.shape

			concate_x[0, :, y_min: y_max, x_min: x_max] = \
				block[:, pad_y_min * upscale: h - pad_y_max * upscale, pad_x_min * upscale: w - pad_x_max * upscale]
	return concate_x



def divide_block(H, W, h_ref=1280, w_ref=720, overlap=20):

	h_block_num = math.ceil(H / h_ref)
	w_block_num = math.ceil(W / w_ref)

	while H % h_block_num != 0: h_block_num += 1
	while W % w_block_num != 0: w_block_num += 1

	h_ideal = H // h_block_num
	w_ideal = W // w_block_num

	blocks_rect = []
	for i in range(h_block_num):
		y_min = max(i * h_ideal - overlap, 0)
		y_max = min((i + 1) * h_ideal + overlap, H)

		for j in range(w_block_num):
			x_min = max(j * w_ideal - overlap, 0)
			x_max = min((j + 1) * w_ideal + overlap, W)

			blocks_rect.append([y_min, y_max, x_min, x_max])

	return blocks_rect, (h_ideal,w_ideal)


def blocks2image_v2(blocks, img_height, img_width, block_rects, model_height, model_width, pad, upscale):
	assert len(blocks) > 0
	assert (img_height % model_height == 0 or model_height > img_height)
	assert (img_width % model_width == 0 or model_width > img_width)
	...
	for i in range(h_reduction_ratio):
		for j in range(w_reduction_ratio):
			...
			concate_x[0, :, y_min: y_max, x_min: x_max] = block[:, pad_y_min * upscale: h - pad_y_max * upscale, pad_x_min * upscale: w - pad_x_max * upscale]

	return concate_x

def divide_block_v2(H, W, h_ref=1280, w_ref=720, overlap=20):
	h_block_num = math.ceil(H / h_ref)
	w_block_num = math.ceil(W / w_ref)
