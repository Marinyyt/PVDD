...
from models.networks.models_rvidenet_srgb_net import RViDeNet, RViDeNet_level
from models.networks.models_no_level_net import FastDVDnet
from models.networks.models_level_net import FastDVDnet_level
from models.networks.models_cnn_net import ModifiedDnCNN, ModifiedDnCNN_level
from models.networks.EDVR_arch_net import EDVR, EDVR_level
...
def test_fastdvdnet(**args):
	...
	# model_temp = EDVR_level()
	# model_temp = FastDVDnet_level()
	# model_temp = ModifiedDnCNN_level()
	model_temp = RViDeNet_level()
	# Load saved weights
	...
	with torch.no_grad():
		files = glob.glob(os.path.join(args['test_path'], 'synNoiseData', 'noisy') + '/*_2_*')[:2]
		psnr_fin = []
		ssim_fin = []
		for idx, file in enumerate(files):
			...
			for j in range(5):
				...
				denframe = torch.zeros((1, 3, H, W))
				rects, _ = divide_block_v2(H, W, H//8, W//8, 20)
				blocks = []
				for rect in rects:
				...
				denframe = blocks2image_v2(blocks, H, W, rects, H//8, W//8, 20, 1)
