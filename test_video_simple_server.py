...
from models.networks.edvrfin_net import EDVRfin
from models.networks.basicVP_net import BasicVP
from models.networks.basicVP_lap_net import BasicVP_lap
from models.networks.basicemvd_net import EMVD
from models.networks.basicemvd_hin_net import EMVD_HIN
from models.networks.basicemvd_lap_net import EMVD_lap
...
def test_fastdvdnet(**args):
	...
	with torch.no_grad():
		videos = glob.glob(args['test_path'] + '/*')

		print(videos)
		for idx, video in enumerate(videos):
			video_name = video.split('/')[-1]
			files = natsorted(glob.glob(video + '/*.png'))[:20]
			print("\tOpen sequence in folder: ", video)
			noise_sigma = 0.0001
			...
			inframes_t = seq.reshape((1, 20, C, H, W)).type(torch.FloatTensor).cuda()
