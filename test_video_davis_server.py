		...
		for dir in dirs:
			if os.path.isdir(dir):
				...
				for i in range(0, len(files) - args['num_frame'] + 1, args['num_frame']):
					...
					s = []
					for j in range(args['num_frame']):
						p.append(compare_psnr(gts[j], res[j].transpose(1, 2, 0), data_range=1.0))
						s.append(compare_ssim(gts[j], res[j].transpose(1, 2, 0), data_range=1.0, multichannel=True))
					psnr_fin.append(np.mean(np.array(p)))
					ssim_fin.append(np.mean(np.array(s)))
					for j in range(args['num_frame']):
						out_name = args['save_path'] + '/' + os.path.basename(dir) + '_' + os.path.basename(lists[j])
