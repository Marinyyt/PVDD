				...
				for sub_dir in sub_dirs:
					if os.path.isdir(sub_dir):
						...
						for i in range(0, len(files)):
							lists = [files[(i + j)%len(files)] for j in range(-(args['num_frame']//2), args['num_frame']//2+1)]
							seq_list = []
							for k in range(len(lists)):
								img = cv2.imread(lists[k].replace('clean', 'noisy0'), -1)
					...
					# print('one processing time' + str(one_end - one_start))
							...
							p = compare_psnr(gt_files[i], res.transpose(1, 2, 0), data_range=1.0)
							s = compare_ssim(gt_files[i], res.transpose(1, 2, 0), data_range=1.0, multichannel=True)
							psnr_fin.append(np.mean(np.array(p)))
							ssim_fin.append(np.mean(np.array(s)))
					# for j in range(5):
							out_name = args['save_path'] + '/' + os.path.basename(dir) + '_' + os.path.basename(sub_dir) + '_' + os.path.basename(files[i])
