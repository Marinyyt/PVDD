...
class VideoDavisDataset(BaseDataset):
    ...
    def __init__(self, config):
        super(VideoDavisDataset, self).__init__(config)
        
        self.root = config['data_path']
        self.num_frame = config['num_frame']
        self.keys = glob.glob(self.root[0] + '/*')
        # print(self.root)
        # print(self.keys)
    ...
    def __getitem__(self, item):
        ...
        # print(folder_name)
        neighbor_list_clean = natsorted(glob.glob(folder_name + '/*.png'))
        start_idx = np.random.randint(0, len(neighbor_list_clean) - self.num_frame)
        
        neighbor_list_clean = neighbor_list_clean[start_idx:start_idx+self.num_frame]
        neighbor_list_noisy = [file.replace('clean', 'noisy') for file in neighbor_list_clean]
