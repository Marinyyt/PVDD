...
from models.networks.pvdd08152_net import pvdd08152
from models.networks.pvdd08153_net import pvdd08153
...
def test_fastdvdnet(**args):
	...
	with torch.no_grad():
		if False:
			dirs = glob.glob(os.path.join(args['test_path'], 'pocNoiseData') + '/*')
			for dir in dirs:
				...
				for sub_dir in sub_dirs:
					if not os.path.isdir(sub_dir):
						continue
					sub_sub_dirs = glob.glob(sub_dir + '/*')
					for sub_sub_dir in sub_sub_dirs:
						if not os.path.isdir(sub_sub_dir):
							continue
						videos = glob.glob(sub_sub_dir + '/*')
						for video in videos:
							if not os.path.isdir(video):
								continue
							files = natsorted(glob.glob(video + '/*.png'))
							gt_file = cv2.imread(files[0], -1)
							if np.max(gt_file) > 256:
								m = 65535
							else:
								m = 255
							for i in range(len(files)):
								lists = [files[(i + j)%len(files)] for j in range(-2, 3)]
								seq_list = []
								for k in range(len(lists)):
									img = cv2.imread(lists[k], -1)
									img = (cv2.cvtColor(img, cv2.COLOR_BGR2RGB)).transpose(2, 0, 1) / m
									seq_list.append(img)
								seq = np.stack(seq_list, axis=0)
								seq = np.clip(seq, 0, 1)	
								seq = torch.from_numpy(seq).type(torch.FloatTensor)
								numframes, C, H, W = seq.shape
								inframes_t = seq.reshape((1, numframes, C, H, W)).type(torch.FloatTensor).cuda()
								noiseLevel = torch.FloatTensor([0.0001])
								denframe = torch.zeros((1, numframes, 3, H, W))
								rects, _ = divide_block_v2(H, W, H // 4, W // 4, 20)
								blocks = [[] for _ in range(numframes)]

								for rect in rects:
									noisemap = noiseLevel.expand((1, 1, rect[1] - rect[0], rect[3] - rect[2])).cuda()
									out = torch.clamp(temp_denoise(model_temp, inframes_t[:, :, :, rect[0]:rect[1], rect[2]:rect[3]], noisemap),0, 1)[0, :, :, :, :]
									for j in range(numframes):
										blocks[j].append(out[j, :, :, :])
								for j in range(args['num_frame']):
									denframe[0, j] = blocks2image_v2(blocks[j], H, W, rects, H // 4, W // 4, 20, 1)
								res = denframe[0, 2].detach().cpu().numpy()
								out_path = os.path.join(args['save_path'], os.path.basename(video))
								if not os.path.exists(out_path):
									os.makedirs(out_path)
								out_name = out_path + '/' + os.path.basename(files[i])
								outimg = np.clip(res.transpose(1, 2, 0) * 255, 0, 255).astype(np.uint8)
		...
		for idx, file in enumerate(files):
			if 'S' in file:
				lists = [file.replace('_2_S.png', '_0_S.png'), file.replace('_2_S.png', '_1_S.png'),
						 file.replace('_2_S.png', '_2_S.png'), file.replace('_2_S.png', '_3_S.png'),
						 file.replace('_2_S.png', '_4_S.png')]
			elif 'M' in file:
				lists = [file.replace('_2_M.png', '_0_M.png'), file.replace('_2_M.png', '_1_M.png'),
						 file.replace('_2_M.png', '_2_M.png'), file.replace('_2_M.png', '_3_M.png'),
						 file.replace('_2_M.png', '_4_M.png')]
			elif 'L' in file:
				lists = [file.replace('_2_L.png', '_0_L.png'), file.replace('_2_L.png', '_1_L.png'),
						 file.replace('_2_L.png', '_2_L.png'), file.replace('_2_L.png', '_3_L.png'),
						 file.replace('_2_L.png', '_4_L.png')]
			...
			for i in range(len(lists)):
				img = cv2.imread(lists[i], -1)
				img = (cv2.cvtColor(img, cv2.COLOR_BGR2RGB)).transpose(2, 0, 1) / m
				seq_list.append(img)

				img = cv2.imread(gts[i], -1)
				img = (cv2.cvtColor(img, cv2.COLOR_BGR2RGB)) / m
				gt_list.append(img)
		...
		for dir in dirs:
			if os.path.isdir(dir):
				files = natsorted(glob.glob(dir + '/img*.png'))
				gt_file = (cv2.cvtColor(cv2.imread(os.path.join(dir, 'clean.png'), -1), cv2.COLOR_BGR2RGB))
				if np.max(gt_file) > 256:
					m = 65535
				else:
					m = 255
				...
				for i in range(0, len(files) - args['num_frame'] + 1, args['num_frame']):
					...
					ssim_fin.append(np.mean(np.array(s)))
					# for j in range(args['num_frame']):
					out_name = args['save_path'] + '/' + os.path.basename(dir) + '.png'
					outimg = np.clip(res[-1].transpose(1, 2, 0) * 255, 0, 255).astype(np.uint8)
					outimg = cv2.cvtColor(outimg, cv2.COLOR_RGB2BGR)
					cv2.imwrite(out_name, outimg)
