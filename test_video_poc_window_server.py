...
def test_fastdvdnet(**args):
	...
	# 	if k == '_metadata':
	# 		continue# 	state_temp_dict_new[k[7:]] = v
	#
	#
	...
	with torch.no_grad():
		dirs = glob.glob(args['test_path'] + '/*')[4:]
		psnr_fin = []
		ssim_fin = []
		for dir in dirs:
			if os.path.isdir(dir):
				...
				for i in range(0, len(files)):
					...
					out_dir = args['save_path'] + '/' + os.path.basename(dir)
					if not os.path.exists(out_dir):
						os.makedirs(out_dir)
					out_name = os.path.join(out_dir, os.path.basename(files[i]))
