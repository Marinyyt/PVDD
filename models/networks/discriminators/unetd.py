...
class UNetDiscriminatorSN(BaseNet):
...
@NETWORK_REGISTRY.register()
class UNetDiscriminatorSimple(BaseNet):
    """A U-Net discriminator with spectral normalization.
    Args:
        in_channels (int): Channel number of the input.
        mid_channels (int, optional): Channel number of the intermediate
    ...
    def __init__(self, in_channels, mid_channels=32, skip_connection=True):
    ...
    def forward(self, img, *args, **kwargs):
        ...
        # upsample
        feat_2 = self.upsample(feat_1)
        feat_3 = self.lrelu(self.conv_6(feat_2))
        if self.skip_connection:
            feat_3 = feat_3 + feat_0

        # final layers
        out = self.lrelu(self.conv_7(feat_3))
