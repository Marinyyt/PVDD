...
@NETWORK_REGISTRY.register()
class EDVR_level(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
                 predeblur=False, HR_in=True, w_TSA=True):
        super(EDVR_level, self).__init__()
        self.nf = nf
        self.center = nframes // 2 if center is None else center
        self.is_predeblur = True if predeblur else False
        self.HR_in = True if HR_in else False
        ...
        '''
        if self.HR_in:
            self.conv_first_1 = nn.Conv2d(3+1, nf, 3, 1, 1, bias=True)
            self.conv_first_2 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
            self.conv_first_3 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
        else:
            self.conv_first = nn.Conv2d(3+1, nf, 3, 1, 1, bias=True)
    ...
    def forward(self, x, noise_map):
        ...
        B, N, C, H, W = x.size()  # N video frames
        x_center = x[:, self.center, 0:3, :, :].contiguous()
