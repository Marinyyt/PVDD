...
class DenBlock2(nn.Module):
	...
	def __init__(self, num_input_frames=3):
		...
		self.downc0 = DownBlock(in_ch=self.chs_lyr0, out_ch=self.chs_lyr1)
		self.downc1 = DownBlock(in_ch=self.chs_lyr1, out_ch=self.chs_lyr2)
		self.residual1 = Residual_Block(in_ch=self.chs_lyr2, out_ch=self.chs_lyr2)
		self.residual2 = Residual_Block(in_ch=self.chs_lyr2, out_ch=self.chs_lyr2)
		self.residual3 = Residual_Block(in_ch=self.chs_lyr2, out_ch=self.chs_lyr2)
		self.upc2 = UpBlock(in_ch=self.chs_lyr2, out_ch=self.chs_lyr1)
		self.upc1 = UpBlock(in_ch=self.chs_lyr1, out_ch=self.chs_lyr0)
		self.outc = OutputCvBlock(in_ch=self.chs_lyr0, out_ch=3)
	...
	def forward(self, in0, in1, in2, noise_map):
		...
		x2 = self.residual1(x2)
		x2 = self.residual2(x2)
		x2 = self.residual3(x2)
...
class FastDVDnet_level(BaseNet):
	""" Definition of the FastDVDnet model.
	Inputs of forward():
		xn: input frames of dim [N, C, H, W], (C=3 RGB)
		noise_map: array with noise map of dim [N, 1, H, W]
	...
	def __init__(self, num_input_frames=5):
		super(FastDVDnet_level, self).__init__()
	...
	def forward(self, x, noise_map):
		...
		# Unpack inputs
		b, n, c, h, w = x.size()
		(x0, x1, x2, x3, x4) = tuple(x[:, i, :, :, :] for i in range(n))
...
class FastDVDnet2(nn.Module):
	...
	def __init__(self, num_input_frames=5):
		super(FastDVDnet2, self).__init__()
		self.num_input_frames = num_input_frames
		# Define models of each denoising stage
		self.temp1 = DenBlock2(num_input_frames=3)
