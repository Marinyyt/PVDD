...
class Predenoiser_level(nn.Module):
    def __init__(self, nf=64):
        ...
        self.conv1_1 = nn.Conv2d(4+1, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2)
...
class RViDeNetraw(BaseNet):
    ...
    def __init__(self, nf=16, nframes=3, groups=1, front_RBs=5, back_RBs=10, center=1):
        super(RViDeNetraw, self).__init__()
        self.center = center
...
@NETWORK_REGISTRY.register()
class RViDeNet_levelraw(BaseNet):
    ## def __init__(self, predenoiser, nf=16, nframes=3, groups=1, front_RBs=5, back_RBs=10, center=1):
    def __init__(self, nf=16, nframes=3, groups=1, front_RBs=5, back_RBs=10, center=1):
        super(RViDeNet_levelraw, self).__init__()
        self.center = center
    ...
    def forward(self, x, noise_map):
        ...
        x_center = x[:, self.center, :, :, :].contiguous()
        ### predenoising
        predenoised_img = self.pre_denoise(x1.view(-1, 4+1, H, W))
