...
import sys
sys.path.append('/mnt/lustre/yuyitong/Deformable-Convolution-V2-PyTorch-pytorch_1.0.0/modules')
import os
import warnings
import math
import torch
...
from  models.modules.op.dcn import ModulatedDeformConv as mDeformConv
# from modulated_deform_conv import ModulatedDeformConv as mDeformConv
...
class DCNv2PackFlowGuided(ModulatedDeformConvPack):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dcnv2 = mDeformConv(self.in_channels, self.out_channels, self.kernel_size, self.stride, self.padding, self.dilation, groups = self.groups, deformable_groups = self.deformable_groups)
...
class VRTs(nn.Module):
    ...
    def __init__(self,
        ...
        self.stage6 = nn.ModuleList(
            [nn.Sequential(
                ...
                nn.Linear(embed_dims[3], embed_dims[4]),
                Rearrange('n d h w c -> n c d h w')
            )]
        )
        for i in range(5, len(depths)):
            self.stage6.append(
    ...
    def forward_features(self, x, flows_backward, flows_forward):
        ...
        x = self.stage4(x3, flows_backward[1::4], flows_forward[1::4])
        x = self.stage5(x + x2, flows_backward[0::4], flows_forward[0::4])
        x = x + x1
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 4, 3, height, width)).to(device)
    x = model(x)
    print(x.shape)
