...
def test_fastdvdnet(**args):
	...
	with torch.no_grad():
		...
		dirs = glob.glob(os.path.join(args['test_path'], 'noisy') + '/*')
		psnr_fin = []
		ssim_fin = []
		for dir in dirs:
			if os.path.isdir(dir):
				files = natsorted(glob.glob(dir + '/*.png'))
				gt_files = [f.replace('noisy', 'clean') for f in files]
				sample = (cv2.cvtColor(cv2.imread(gt_files[0], -1), cv2.COLOR_BGR2RGB))
				if np.max(sample) > 256:
					m = 65535
				else:
					m = 255
				for i in range(0, len(files)-args['num_frame']+1):
					lists = [files[i + j] for j in range(args['num_frame'])]
					gt_lists = [gt_files[i + j] for j in range(args['num_frame'])]
					...
					for k in range(len(lists)):
						...
						gt = cv2.imread(gt_lists[k], -1)
						gt = (cv2.cvtColor(gt, cv2.COLOR_BGR2RGB)) / m
						gts.append(gt)
					...
					rects, _ = divide_block_v2(H, W, H // 4, W // 2, 20)
					blocks = []
					...
					denframe = blocks2image_v2(blocks, H, W, rects, H // 4, W // 2, 20, 1)
					res = denframe[0].detach().cpu().numpy()
					p = compare_psnr(gts[args['num_frame']//2], res.transpose(1, 2, 0), data_range=1.0)
					s = compare_ssim(gts[args['num_frame']//2], res.transpose(1, 2, 0), data_range=1.0, multichannel=True)
					psnr_fin.append(np.mean(np.array(p)))
					ssim_fin.append(np.mean(np.array(s)))
					# for j in range(5):
					out_name = args['save_path'] + '/noisy_' + os.path.basename(dir) + '_' + os.path.basename(lists[0])
					outimg = np.clip(res.transpose(1, 2, 0) * 255, 0, 255).astype(np.uint8)
					outimg = cv2.cvtColor(outimg, cv2.COLOR_RGB2BGR)
					cv2.imwrite(out_name, outimg)
					#
					# out_name = args['save_path'] + '/clean_' + os.path.basename(lists[0])
					# outimg = np.clip(gts[args['num_frame']//2] * 255, 0, 255).astype(np.uint8)
					# outimg = cv2.cvtColor(outimg, cv2.COLOR_RGB2BGR)
					# cv2.imwrite(out_name, outimg)
		print('DAVIS PSNR: ' + str(np.mean(np.array(psnr_fin))))
		print('DAVIS SSIM: ' + str(np.mean(np.array(ssim_fin))))
